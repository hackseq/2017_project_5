...
The follwoing is the authetic code in my on-going project to find the key 
players in controling left-right (L-R) patterning during heart development.

My biologoical question is, what are the novel determint involved in LR patterning?
...
GSE85728.rsubread.gencode & gencode.GRCh38.mouse) generated from the above protocol
to show the inference procedure.
...
GSE85728.vsd.data <- GSE85728.rsubread.gencode %$% counts %>% 
                    DESeqDataSetFromMatrix( countData = ., 
                                           ...
                                           design = ~ tissueGroups) %>%
                    DESeq() %>% getVarianceStabilizedData() %>% invisible()
