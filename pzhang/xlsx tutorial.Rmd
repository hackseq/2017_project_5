...
```{R}
  ...
  expr.data<-read.csv("exprData.csv.gz")
  expr.gene<-as.data.frame(expr.data[gene,])
```
...
```{R}
  ...
  gene.boxplot<-ggplot(expr.gene, aes(x = STAGE, y = log2(Expression), fill=STAGE)) +
    geom_boxplot() +
    labs(x = "Tumor Stage",
...
```{R}
  ...
  addPicture("Expression.figure.png", sheet1, scale=1, startRow=31, startColumn=12)
```  
